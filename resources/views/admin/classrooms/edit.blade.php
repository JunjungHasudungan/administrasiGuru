            ...
            <form action="{{route('admin.classrooms.update', [$classroom->id] )}}" method="POST" enctype="multipart/form-data">
                      ...
                      <div class="relative">
                        <select id="teacher_id" name="teacher_id" class="form-multiselect  block capitalize appearance-none w-full bg-gray-200 border border-gray-200 text-gray-700 py-3 px-4 pr-8 rounded leading-tight focus:outline-none focus:bg-white focus:border-gray-500 form-control {{ $errors->has('teacher_id') ? 'is-invalid' : '' }}"  >
                            @foreach ($teachers as $id => $teacher)
                                <option class="font-normal hover:font-bold capitalize" value="{{$id}}" {{  ($classroom->homeworkTeacher ? $classroom->homeworkTeacher->id : old('teacher_id')) == $id ? 'selected' : ''}}>{{$teacher}}</option>
                            @endforeach
                        </select>
                            @error('teacher')
                ...
                <div class="flex flex-wrap -mx-3 mb-2 mt-5">
                      ...
                      <div class="relative">
                        <select id="major_id" name="major_id" class="form-multiselect  block capitalize appearance-none w-full bg-gray-200 border border-gray-200 text-gray-700 py-3 px-4 pr-8 rounded leading-tight focus:outline-none focus:bg-white focus:border-gray-500 form-control {{ $errors->has('major_id') ? 'is-invalid' : '' }}"  >
                            @foreach ($majors as $id => $major)
                                <option class="font-normal hover:font-bold capitalize" value="{{$id}}" {{ ( $classroom->major ? $classroom->major->id : old('major_id')) == $id ? 'selected' : ''}}>
                                    {{$major}}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="w-full md:w-1/3 px-3 mb-6 md:mb-0">
                        ...
                        <input id="room_number" type="text" name="room_number"  value="{{old('room_number', $classroom->room_number) }}" class=" form-input appearance-none block w-full bg-gray-200 text-gray-700 border border-gray-200 rounded py-3 px-4 leading-tight focus:outline-none focus:bg-white focus:border-gray-500 {{ $errors->has('room_number') ? 'is-invalid' : '' }}" required>
